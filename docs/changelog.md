...
- **主题配色重构（做减法）**：移除观感不稳定的 `玫瑰粉 / 棉花糖 / 薰衣草 / 蜜桃` 预设，仅保留精选高质感主题。
- **新增紫晶主题**：新增 `紫晶 (Violet)` 作为偏现代的紫系高对比方案。
- **全量重调色板**：对保留主题（靛蓝、石墨灰、翡翠绿、暖日橙、午夜蓝、海洋蓝）的亮色/暗色参数进行重新校准，提升耐看度与一致性。
- **兼容旧用户配置**：历史 theme 值自动映射到新主题（例如 `rose -> violet`），避免升级后配置失效。
- **换肤样式减法与重构**：移除重复感较强的 `极简 / 鲜明`，新增形态差异更明显的 `棱镜 / 科幻 HUD`。
- **控件形态增强**：`棱镜` 对按钮、输入框、菜单等基础控件启用斜切几何外形；`科幻 HUD` 强化玻璃质感、发光边框与高科技界面层次。
- **旧换肤兼容映射**：历史 `minimal -> sharp`、`vivid -> hud` 自动迁移，保证升级后配置可用。
- **换肤继续收敛**：移除 `紧凑 / 粗野`，并兼容历史值映射到 `硬朗`。
- **棱镜适配修正**：取消大范围 `clip-path` 造成的布局/交互问题，改为更稳定的斜角圆角表达。
- **胶囊形态修正**：降低椭圆弧度，避免过度“鼓包”与内容包裹不协调。
- **新增赛博霓虹皮肤**：增加 `Cyber` 科幻风皮肤，提供霓虹描边、扫描线层与 HUD 边角轮廓，整体更偏电影级高科技观感。
- **统一页面标题字号**：将中转站、API Key、Codex、开发者工具、日历、设置等页面 header 标题统一收敛到全局 token，避免字号不一致。
- **新增 Cyber Darkline 皮肤**：增加更暗、更冷、更偏电影控制台观感的 `Cyber Darkline`，强化冷色网格线、扫描线与高亮边框层次。
