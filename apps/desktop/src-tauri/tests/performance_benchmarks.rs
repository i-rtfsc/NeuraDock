...
async fn benchmark_account_queries() {
    ...
    println!("✓ find_all() (empty): {:.2}ms, {} accounts", 
        elapsed.as_secs_f64() * 1000.0, accounts.len());
    assert_eq!(accounts.len(), 0);
    
    // Benchmark 2: find_by_id() with non-existent ID
    let start = Instant::now();
    let account = repo.find_by_id(
        &neuradock_lib::domain::shared::AccountId::from_string("non-existent")
    ).await.unwrap();
    let elapsed = start.elapsed();
    println!("✓ find_by_id() (not found): {:.2}ms, found: {}", 
        elapsed.as_secs_f64() * 1000.0, account.is_some());
    ...
    let elapsed = start.elapsed();
    println!("✓ find_enabled() (empty): {:.2}ms, {} accounts", 
        elapsed.as_secs_f64() * 1000.0, enabled_accounts.len());
...
async fn benchmark_encryption_performance() {
    ...
    println!("✓ Encryption: {:.2}µs per operation (avg over {} iterations)",
        encrypt_avg * 1_000_000.0, iterations);
    ...
    println!("✓ Decryption: {:.2}µs per operation (avg over {} iterations)",
        decrypt_avg * 1_000_000.0, iterations);
    
    println!("\n📊 Summary:");
    println!("   - Encryption throughput: {:.2} ops/sec", 1.0 / encrypt_avg);
    println!("   - Decryption throughput: {:.2} ops/sec", 1.0 / decrypt_avg);
    println!("========================================================\n");
}
...
fn benchmark_account_aggregate_creation() {
    ...
    for i in 0..iterations {
        let cookies = HashMap::from([
            ("session".to_string(), format!("session_{}", i)),
        ]);
    ...
    println!("✓ Account aggregate creation: {:.2}µs per operation (avg over {} iterations)",
        avg * 1_000_000.0, iterations);
